...
void LargeParkLevel::OnInitialized()
{
	super::OnInitialized();

	// 이 레벨에서 사용할 맵 불러오기
	LoadMap("LargeParkMap.txt");
...
void LargeParkLevel::StartLeafEffect()
{
	onLeafEffect = true;
}

void LargeParkLevel::EndLeafEffect()
{
	onLeafEffect = false;
}

void LargeParkLevel::AddCGarbage()
{
	++garbageCount;
...
void LargeParkLevel::ClearStage()
{
	if (!isStageCleared)
	{
		return;
...
bool LargeParkLevel::IsStageClear() const
{
	return isStageCleared;
}

void LargeParkLevel::LoadMap(const std::string& filename)
{
	// 경로 조립 ( 상대 경로 지정 )
